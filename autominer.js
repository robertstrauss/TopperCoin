...
async function fromBlock(lastblock) {
  ...
  con.log('mining', blockstring)
  thisNode.miner.postMessage([blockstring, difficulty]); // send data for mining
  thisNode.miner.addEventListener('message', e => socket.emit('block', e.data)); // emit block when done
  thisNode.miner.addEventListener('message', e=>con.log('miner sent', e.data));
}
